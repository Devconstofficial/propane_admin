String kAppName = 'PROPANEGO';
String kWelcomeText = 'Welcome to the PROPANEGO Admin Panel 👋';
String kWelcomeDetail = 'Log in to manage and oversee user and driver activities, track deliveries, and ensure smooth operations with ease.';
String kRemember = 'Remember Me';
String kFirstTimeFreeDel = 'First Time Free Delivery';
String kCanPayCash = 'Can Pay In Cash';
String kLogin = 'Login';
String kVerify = 'Verify';
String kTwoFactor = 'Two Factor Authentication';
String kTwoFactorDetail = 'For enhanced security, please enter the Google authentication code send to your email or app.';
String kToday = 'Today';
String kActiveUsers = 'Active Users';
String kActiveUser = 'Active User';
String kCreateAdmin = 'Create Admin';
String kCreateDriver = 'Create Driver';
String kCreateNewUser = 'Create New User';
String kCreateNewDriver = 'Create New Driver';
String kActiveJobDetails = 'Active Job Details';
String kPlatformFee = 'Platform Usage Fee';
String kSelectRegion = 'Select Region';
String kSelectRegionHint = 'Puerto Plata / Santo Domingo';
String kPromotionType = 'Promotion Type';
String kPromotionTypeHint = 'Full off / Specific off';
String kDuration = 'Duration';
String kDurationHint = 'Specific ID / All';
String kPromotionalTitle = 'Promotional Title';
String kPromotionalTitleHint = 'Puerto Plata / Santo Domingo';
String kDiscountType = 'Discount Type';
String kDiscountAmount = 'Discount Amount';
String kSpecifyUser = 'Specify User';
String kSpecifyUserHint = 'Specific ID / All';
String kDiscountTypeHint = 'percentage or fixed amount';
String kTransactionFee = 'Transaction Fee';
String kDeliveryFee = 'Set Delivery Fee';
String kUrgentPrice = 'Urgent Price';
String kUltraUrgent = 'Ultra Urgent';
String kBookingPrice = 'Booking Price';
String kRevenue = 'Revenue This Month';
String kBeverageManagement = 'Beverage Management';
String kBookingThisWeek = 'Bookings This Week';
String kActiveAdmins = 'Active Admins';
String kRevenueText = 'Revenue';
String kUserAnalytics = 'User Analytics';
String kDeliveryTrends = 'Delivery Trends';
String kLocationBasedService = ' Location-Based Service';
String kUserPayment = 'User Payment';
String kDriverPayout = 'Driver Payout';
String kFinancialTrans = 'Financial and Transaction Management';
String kAgeRange = 'Age Range';
String kEmploymentStatus = 'Employment Status';
String kRevenueLocation = 'Revenue By Location';
String kCrimeTypeText = 'Crime Type';
String kSpecifyRegion = 'Specify Region';
String kFeePercentage = 'Fee Percentage';
String kFlagDriver = 'Flag Driver?';
String kSubmitFlag = 'Submit Flag';
String kFlagDriverDetail = 'Are you sure you want to flag this driver? This action will notify the admin team to investigate any reported issues or concerns. Flagging a driver should only be done for valid reasons, such as misconduct or repeated service failures.';
String kPricePerMile = 'Price per mile';
String kPricePerGallon = 'Price per gallon';
String kSendReminder = 'Send Reminder';
String kSendReminderDetail = 'Would you like to send a reminder to the user regarding their gas level or subscription renewal? Choose the type of reminder and customize the message if needed.';
String kWeeklyReported = 'Weekly Reported Crimes';
String kUserManagement = 'User Management';
String kAdminManagement = 'Admin Management';
String kDriverManagement = 'Driver Management';
String kAnalyticsReporting = 'Analytics and Reporting';
String kJobManagement = 'Job Management';
String kSensorDetails = 'Sensor Details';
String kSubscriptionManagement = 'Subscription Management';
String kFilterBy = 'Filter By';
String kUserID = 'User ID';
String kName = 'Name';
String kReportsPosted = 'Reports Posted';
String kStatus = 'Status';
String kActions = 'Actions';
String kAddCrime = 'Create Admin';
String kLocation = 'Location';
String kDateTime = 'Date & Time';
String kPassword = 'Password';
String kAddress = 'Address';
String kComments = 'Comments';
String kSubscriptionType = 'Subscription Type';
String kCategory = 'Category';
String kDate = 'Date';
String kPushNotifications = 'Push Notifications';
String kCreateNotifications = 'Create notifications';
String kUserName = 'User Name';
String kFullName = 'Full Name';
String kEmail = 'Email';
String kSeverity = 'Severity';
String kDateTimeIncident = 'Date and time of the incident';
String kConfirmPassword = 'Confirm Password';
String kPhoneNo = 'Phone No';
String kRole = 'Role';
String kOccupation = 'Occupation';
String kVehicleType = 'Vehicle Type';
String kPickupLocation = 'Pickup Location';
String kDeliveryLocation = 'Delivery Location';
String kDeliveryCharges = 'Delivery Charges';
String kGenerateReport = 'Generate Report';
String kPlatformCommission = 'Platform Commission';
String kSchPickTime = 'Scheduled Pickup Time';
String kVehicleLiPlate = 'Vehicle License Pate';
String kEmpStatus = 'Employment Status';
String kAge = 'Age';
String kBeverages = 'Beverages';
String kAddBeverage = 'Add Beverage';
String kCustomerRequests = 'Customer Requests';
String kActiveChats = 'Active Chats';
String kNewMessage = 'New Message';
String kFirstTimeDelivery = 'First Time Free Delivery';
String kDiscountManagement = 'Discount Management';
String kRegionalPromotion = 'Regional Promotion';
String kSeasonalPromotion = 'Seasonal Promotion';
String kReferralPromotion = 'Referral Promotion';
String kPopUps = 'Pop-Ups';
String kAddCoupon = 'Add Coupon';
String kDisableAll = 'Disable All';
String kActive = 'Active';
String kPending = 'Pending';
String kRejected = 'Rejected';
String kDisabled = 'Disabled';
String kDeleteDetail = 'Are you sure you want to delete this item? This action is irreversible and will permanently remove the selected record from the system.';


// Routs
const String kLoginScreenRoute = "/";
const String kVerifyOtpScreenRoute = "/verify-otp-scree-route";
const String kDashboardScreenRoute = "/dashboard-screen-route";
const String kUserScreenRoute = "/user-screen-route";
const String kDriverScreenRoute = "/driver-screen-route";
const String kJobManagementScreenRoute = "/job-screen-route";
const String kJobDetailScreenRoute = "/job-detail-screen-route";
const String kAdminScreenRoute = "/admin-screen-route";
const String kAddAdminScreenRoute = "/add-admin-screen-route";
const String kCreateUserScreenRoute = "/create-user-screen-route";
const String kCreateDriverScreenRoute = "/create-driver-screen-route";
const String kBeverageScreenRoute = "/beverage-screen-route";
const String kSupportScreenRoute = "/support-screen-route";
const String kChatScreenRoute = "/chat-screen-route";
const String kRevenueScreenRoute = "/revenue-screen-route";
const String kAnalyticsScreenRoute = "/analytics-screen-route";
const String kPromotionScreenRoute = "/promotion-screen-route";


